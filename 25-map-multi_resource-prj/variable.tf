...
variable "ec2_instance_config_map" {
  ...
  validation {
    condition = alltrue([
      ...
      # for key, cfg in values(var.ec2_instance_config_map) : contains(["t2.micro"], cfg.instance_type)
    ])
  ...
  validation {
    condition = alltrue([
      ...
      # for key, cfg in values(var.ec2_instance_config_map) : contains(["ubuntu", "nginx"], cfg.ami)
    ])
